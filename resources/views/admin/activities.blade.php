            ...
            <div style="grid-column:1/-1">
                ...
                <div id="form-cover-existing" class="file-existing" style="display:none;margin-top:0.5rem">
                    <div class="fe-img-thumb" onclick="previewImage($(this).find('img').attr('src'))">
                        ...
                        <div class="fe-img-overlay"><i class="fa-solid fa-expand" style="font-size:12px"></i></div>
                    </div>
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            $.each(json.data, function (i, act) {
                ...
                var statusHtml = act.status ? '<span class="badge badge-success">เปิดใช้งาน</span>' : '<span class="badge badge-warning">ปิดใช้งาน</span>';
                ...
                tbody.append('<tr>' +
                    ...
                    '<td><div style="display:flex;align-items:center;gap:1rem"><div style="width:48px;height:48px;border-radius:var(--radius);background:linear-gradient(135deg,#667eea,#764ba2);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;flex-shrink:0">' + initial + '</div><div><p style="font-weight:600;margin-bottom:2px">' + titleSafe + '</p></div></div></td>' +
...
function viewActivity(id) {
    $.ajax({
        ...
        success: function (json) {
            ...
            $('#view-status').html(act.status ? '<span class="badge badge-success">เปิดใช้งาน</span>' : '<span class="badge badge-warning">ปิดใช้งาน</span>');
            if (act.cover_image_url) { $('#view-cover').attr('src', act.cover_image_url); $('#view-cover-wrap').show(); } else { $('#view-cover-wrap').hide(); }
            if (act.pdf_url) { $('#view-pdf').attr('src', act.pdf_url); $('#view-pdf-wrap').show(); } else { $('#view-pdf-wrap').hide(); }
        ...
        error: function (xhr) {
            ...
            if (xhr.responseJSON && xhr.responseJSON.errors) {
                ...
                if (typeof errs === 'object') {
                    var lines = [];
                    $.each(errs, function (k, v) { lines.push(v); });
                    msg = lines.join('\n');
                }
            }
            showToast(msg, 'error');
